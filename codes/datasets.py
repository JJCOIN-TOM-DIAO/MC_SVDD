...
class Multi_Center_Dataset(Dataset):
    def __init__(self, memmap, K=64, repeat=1):
        super().__init__()
        self.arr = np.asarray(memmap)
        self.K = K
    ...
    def __getitem__(self, idx):
        ...
        p1 = patch_generator(256, 256, K)

        image = self.arr[n]


        patch = crop_image_CHW(image, p1, K)


        return patch

    @staticmethod
    def infer(enc,multi_cls, batch):
        x1s= batch
        h1s = enc(x1s)
        loss = multi_cls(h1s)
        return loss
...
class Multi_Center_Dataset_res(Dataset):
    def __init__(self, memmap, K=256, repeat=1):
        super().__init__()
        self.arr = np.asarray(memmap)
        self.K = K
        self.repeat = repeat
    ...
    def __getitem__(self, idx):
        ...
        p1 = patch_generator(1024, 1024, K)
